...
| `DATABASE_SSL_CA_CERT` | DB サーバー CA 証明書のパス（任意） | `/etc/ssl/certs/ca.pem` |
...
マネージド PostgreSQL では `DATABASE_SSL=true` を設定する。CA 検証が必要な場合は `DATABASE_SSL_CA_CERT` に証明書パスを指定する。
