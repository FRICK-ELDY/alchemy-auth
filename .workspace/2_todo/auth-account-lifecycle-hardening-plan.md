...
- [ ] `email_verified_at` 相当の状態管理を導入する
- [ ] 確認トークンの発行、検証、期限切れ処理を定義する
- [ ] パスワードリセット API の入出力とトークン寿命を決める
- [ ] 認証済みユーザー向けのパスワード変更 API を追加する
- [ ] 退会時の token revoke とデータ保持方針を決める
- [ ] register 失敗レスポンスから存在有無を直接推測しにくい形へ整える
- [ ] 正常系と期限切れ / 不正 token 系のテストを追加する
...
- [ ] 他人メールで登録しても確認完了前は所有証明済みにならない
- [ ] パスワード忘れ、変更、退会の導線が API として揃う
- [ ] register 応答の列挙耐性が改善される
- [ ] 主要フローにテストがある
