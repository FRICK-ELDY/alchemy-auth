...
- [ ] 採用ライブラリまたは自前実装方針を決める
- [ ] API ごとの key と window を定義する
- [ ] 開発環境で緩和、本番で厳格化できる設定を追加する
- [ ] 429 応答のエラーフォーマットを既存 API と整合させる
- [ ] 正常系、超過系、境界値のテストを追加する
...
- [ ] `login` / `register` / `refresh` 全てでレート制限が有効
- [ ] 制限超過時に 500 ではなく 429 を返す
- [ ] テストで超過時の挙動を確認できる
- [ ] 本番向けの閾値を設定で変更できる
